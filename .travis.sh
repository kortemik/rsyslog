...
PKG_CONFIG_PATH="${HOME}/build/kortemik/target/librelp/lib/pkgconfig" CFLAGS="-I${HOME}/build/kortemik/target/liblogging/include" LIBESTR_CFLAGS="-I${HOME}/build/kortemik/target/libestr/include" LIBESTR_LIBS="-L${HOME}/build/kortemik/target/libestr/lib -lestr" LIBLOGGING_STDLOG_CFLAGS="-I${HOME}/build/kortemik/target/liblogging/include" LIBLOGGING_STDLOG_LIBS="-L${HOME}/build/kortemik/target/liblogging/lib -llogging-stdlog" LIBFASTJSON_CFLAGS="-I${HOME}/build/kortemik/target/libfastjson/include/libfastjson" LIBFASTJSON_LIBS="-L${HOME}/build/kortemik/target/libfastjson/lib -lfastjson" RELP_CFLAGS="-I${HOME}/build/kortemik/target/librelp/include" RELP_LIBS="${HOME}/build/kortemik/target/librelp/lib" ./autogen.sh --prefix=${HOME}/build/kortemik/target/rsyslog --disable-generate-man-pages --enable-relp --enable-imfile
make
make install
